...
def from_whatever(thing,create_using=None,multigraph_input=False):
    ...
    if hasattr(thing,"adj"):
        try:
            ...
            raise networkx.NetworkXError("Input is not a correct NetworkX graph.")
    ...
    if hasattr(thing,"is_strict"):
        try:
            ...
            raise networkx.NetworkXError("Input is not a correct pygraphviz graph.")
    ...
    if isinstance(thing,list) or hasattr(thing,'next'): 
        try:
            ...
            raise networkx.NetworkXError("Input is not a valid edge list")
    ...
    try:
        ...
        if isinstance(thing,numpy.core.defmatrix.matrix) or \
            ...
            try:
                ...
                raise networkx.NetworkXError("Input is not a correct numpy matrix or array.")
    except ImportError:
    ...
    try:
        ...
        if hasattr(thing,"format"):
            try:
                ...
                raise networkx.NetworkXError("Input is not a correct scipy sparse matrix type.")
    except ImportError:
        warnings.warn('scipy not found, skipping conversion test.',
                      ImportWarning)


    raise networkx.NetworkXError("Input is not a known data type for conversion.")
...
def from_numpy_matrix(A,create_using=None):
    ...
    if nx!=ny:
        raise networkx.NetworkXError( "Adjacency matrix is not square. nx,ny=%s"%(A.shape,))
...
def to_scipy_sparse_matrix(G, nodelist=None, dtype=None):
    ...
    try:
        ...
        raise ImportError("to_scipy_sparse_matrix() requires scipy: http://scipy.org/ ")
    ...
    undirected = not G.is_directed()
    index=dict(zip(nodelist, range(nlen)))
    names_index = {u: v for v, u in index.items()}
    ...
    for u, v, attrs in G.edges(data=True):
        if (u in nodeset) and (v in nodeset):
            ...
            if attrs.get('weight') == 0:
                attrs['weight'] = 0.0001
            M[i, j] += attrs.get('weight', 0.0001)
            if undirected:
                M[j, i] = M[i, j]
        else:
            print(u, v)
    return M, index, names_index
